...
class maint_insuranceCompanyController extends Controller
{
	...
	public function __construct(clientListConnection $clist, contactPersonConnection $comPerson, inscompanyConnection $comp, addressConnection $add, personalInfoConnection $personalinfo)
    {
        $this->list = $clist;
        $this->company = $comp;
        $this->address = $add;
    ...
    {
      return view('/pages/admin/maintenance/insurance company')
    ...
    {
      $this->list->client_type = 0;
      $this->list->client_flag = 0;
      $mytime = $req->time;
	...
	{
		$compy = clientListConnection::where('client_ID', '=', $req->bnkid)->first();
	...
	public function ardelete_inscomp(Request $req)
	{
      ...
      {
				$compy = clientListConnection::where('client_ID', '=', $ID)->first();
