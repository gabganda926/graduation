...
class trans_ListCVController extends Controller
{

    public function index()
    {
      return view('pages.accounting-staff.transaction.list-check-voucher')
    ...
    public function generateCV(Request $request, $cv_ID, $pinfo_ID, $account_ID) 
    {
        ...
        $pdf = PDF::loadView('pages.pdf.check-voucher',
    ...
    public function generateCVcomp(Request $request, $cv_ID, $comp_ID, $account_ID) 
    {
        $cv = checkVoucherConnection::where('cv_ID',$cv_ID)->first();
        $pno = clientAccountsConnection::where('account_ID',$account_ID)->first();
        ...
        $pdf = PDF::loadView('pages.pdf.check-voucher-comp',
                compact('cv', 'pno', 'inf', 'list', 'payDet', 'bank', 'address', 'inscomp'))
            ->setPaper('Letter');
