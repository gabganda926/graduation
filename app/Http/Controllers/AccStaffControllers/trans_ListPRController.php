...
class trans_ListPRController extends Controller
{
    public function index()
    {
      return view('pages.accounting-staff.transaction.list-policy-receipt')
    ...
    public function generatePR(Request $request, $cv_ID, $pinfo_ID, $account_ID) 
    {
        ...
        $pdf = PDF::loadView('pages.pdf.policy-receipt',
    ...
    public function generatePRcomp(Request $request, $cv_ID, $comp_ID, $account_ID) 
    {
        $cv = checkVoucherConnection::where('cv_ID',$cv_ID)->first();
        $pno = clientAccountsConnection::where('account_ID',$account_ID)->first();
        ...
        $pdf = PDF::loadView('pages.pdf.policy-receipt-comp',
                compact('cv', 'pno', 'inf', 'list', 'payDet', 'bank', 'address', 'inscomp'))
            ->setPaper('Letter');
