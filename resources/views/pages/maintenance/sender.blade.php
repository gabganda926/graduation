                            ...
                            <table class="table table-bordered table-striped table-hover js-basic-example dataTable animated lightSpeedIn active">
                                ...
                                <tbody>
                                  <?php
                                    $index = 1;
                                ?>
                                    @foreach($snd as $snddata)
                                     @if($snddata->del_flag == 0)
                                      @foreach($pnf as $Info)
                                        @if($Info->pinfo_ID == $snddata->personal_info_ID)
                                    <tr>
                                        <td><input type="checkbox" id="{{ $snddata->sender_ID }}" name = "del_check" class="filled-in chk-col-red checkCheckbox"/>
                                        <label for="{{ $snddata->sender_ID }}"></label></td>
                                        <td>
                                        ...
                                        <td>{{ \Carbon\Carbon::parse($snddata->created_at)->format('M-d-Y') }} <br/> {{ "(".\Carbon\Carbon::parse($snddata->created_at)->format('l, h:i:s A').")" }}</td>
                                        <td>{{ \Carbon\Carbon::parse($snddata->updated_at)->format('M-d-Y') }} <br/> {{ "(".\Carbon\Carbon::parse($snddata->updated_at)->format('l, h:i:s A').")" }}</td>
                                        <td>
                                        <button type="button" class="btn bg-light-blue waves-effect" data-toggle="modal" data-target="#largeModal"
                                        data-empid='{{ $snddata->sender_ID }}'
                                        ...
                                        data-pinfo='{{ $snddata->personal_info_ID }}'
            ...
            <div class="modal fade" id="largeModal" tabindex="-1" role="dialog">
                <div class="modal-dialog modal-lg animated zoomInRight active" role="document">
                    <div class="modal-content">
                        ...
                        onclick = "
                        document.getElementById('emp_view').action = 'sender/delete';
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group form-float">
                                            ...
                                            <label><small>Email :</small></label>
                                                <input id = "aemp_email" name = "aemp_email" type="email" class="form-control" name="email" disabled="disable" >
                                          </div>
                                        </div>
                                    </div>
                                </div>
            ...
            <div class="modal fade" id="addEmpModal" tabindex="-1" role="dialog">
                <div class="modal-dialog modal-lg animated zoomInLeft active" role="document">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <form id="emp_add" name = "emp_add" action = "sender/submit" method="POST">
                              <input type="hidden" name="_token" value="{{ csrf_token() }}">
